...
class VLCPlayerWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.volume_slider.valueChanged.connect(
            lambda val: self.mplayer.audio_set_volume(val)
        )

        # Subtitles toggle
        self.subsbutton = QtWidgets.QPushButton("Subtitles: Off")
        self.subsbutton.setCheckable(True)
        self.subsbutton.clicked.connect(self.toggle_subtitles)
        ...
        self.hbox.addWidget(self.subsbutton)
        ...
        self.inactivity_timer.timeout.connect(
            lambda: self.controls.setVisible(False)
        )
    ...
    """def toggle_subtitles(self):
        ...
        try:
            if self.subsbutton.isChecked():
                tracks = self.mplayer.video_get_spu_description()
                if tracks and len(tracks) > 0:
                    # pick the first available subtitle track
                    first_track_id = tracks[0][0]
                    self.mplayer.video_set_spu(first_track_id)
                    self.subsbutton.setText("Subtitles: On")
                else:
                    self.subsbutton.setChecked(False)
                    self.subsbutton.setText("Subtitles: Off")
            else:
                self.mplayer.video_set_spu(-1)  # disable subs
                self.subsbutton.setText("Subtitles: Off")
        except Exception as e:
            print("Subtitle toggle failed:", e)
            self.subsbutton.setChecked(False)
            self.subsbutton.setText("Subtitles: Off")"""
    def toggle_subtitles(self):
        if not self.mplayer:
        ...
        try:
            if self.subsbutton.isChecked():
                tracks = self.mplayer.video_get_spu_description()
                if tracks:
                    # Skip the "Disable" track (-1) and pick the first real subtitle
                    valid_tracks = [t for t in tracks if t[0] != -1]
                    if valid_tracks:
                        track_id, track_name = valid_tracks[0]
                        self.mplayer.video_set_spu(track_id)
                        self.subsbutton.setText(f"Subtitles: On ({track_name})")
                        return
                # If no subs
                self.subsbutton.setChecked(False)
                self.subsbutton.setText("Subtitles: Off")
            else:
                self.mplayer.video_set_spu(-1)  # disable
                self.subsbutton.setText("Subtitles: Off")
